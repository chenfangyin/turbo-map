...
class ErrorRecoveryManager {
    ...
    constructor() {
        this.errorStats = {
            total: 0,
            byType: Object.fromEntries(Object.values(ErrorType).map(type => [type, 0])),
            byOperation: {},
            recoveryActions: Object.fromEntries(Object.values(RecoveryAction).map(action => [action, 0]))
        };
    ...
    handleError(error, operation, errorType = ErrorType.UNKNOWN) {
        try {
    ...
    executeWithRecovery(operation, fallback, operationName, errorType = ErrorType.UNKNOWN) {
        try {
            return operation();
        }
        catch (error) {
    ...
    async executeAsyncWithRecovery(operation, fallback, operationName, errorType = ErrorType.UNKNOWN) {
        try {
...
class AsyncTurboMap {
    ...
    async batchExecute(operations) {
        return globalErrorRecovery.executeAsyncWithRecovery(async () => {
            ...
            return [];
        }, 'batchExecute', ErrorType.UNKNOWN);
    }
    ...
    async deleteAllAsync(keys) {
        return globalErrorRecovery.executeAsyncWithRecovery(async () => {
            ...
            return [];
        }, 'deleteAllAsync', ErrorType.UNKNOWN);
    }
    async sizeAsync() {
        return globalErrorRecovery.executeAsyncWithRecovery(async () => {
            await this.nextTick();
...
class TurboMapAsyncStream {
    ...
    async forEach(callback) {
        ...
        }, 'streamForEach', ErrorType.ITERATION);
    }
    async collect() {
        return globalErrorRecovery.executeAsyncWithRecovery(async () => {
            ...
            return new Map();
        }, 'streamCollect', ErrorType.UNKNOWN);
    }
    async toArray() {
        return globalErrorRecovery.executeAsyncWithRecovery(async () => {
            return [...this.entries];
            ...
            return [];
        }, 'streamToArray', ErrorType.UNKNOWN);
    }
    async reduce(callback, initialValue) {
        return globalErrorRecovery.executeAsyncWithRecovery(async () => {
            ...
            return initialValue;
        }, 'streamReduce', ErrorType.UNKNOWN);
    }
    async delay(ms) {
        return new Promise(resolve => safeSetTimeout(resolve, ms));
    }
...
class PluginManager {
    ...
    async disablePlugin(pluginName) {
        return globalErrorRecovery.executeAsyncWithRecovery(async () => {
            ...
            return false;
        }, 'disablePlugin', ErrorType.PLUGIN);
    }
    ...
    handlePluginError(plugin, error, context) {
        ...
        if (this.options.enableErrorRecovery) {
            const action = globalErrorRecovery.handleError(error, `plugin:${plugin.name}:${context.operation}`, ErrorType.PLUGIN);
...
function createEnhancedTurboMap(entriesOrOptions, optionsParam) {
    ...
    return globalErrorRecovery.executeWithRecovery(() => createEnhancedTurboMapInternal(entries, config), () => createFallbackEnhancedTurboMap(), 'createEnhancedTurboMap', ErrorType.UNKNOWN);
}
function createEnhancedTurboMapInternal(entries, config) {
    ...
    const serialize = (key) => {
        ...
        }, 'serialize', ErrorType.SERIALIZATION);
    };
    const enhancedMap = {
...
export { AdaptiveSerializer, AsyncTurboMap, DiagnosticAnalyzer, EnhancedLRUCache, EnhancedObjectPool, ErrorRecoveryManager, ErrorType, FastHasher, MemoryAnalyzer, ObjectPool, PerformanceMonitor, PluginManager, RecoveryAction, TieredCacheManager, TypeUtils, createEnhancedTurboMap, createEnhancedTurboMap as createTurboMap, globalDiagnostic, globalErrorRecovery, globalFastHasher, globalObjectPool, globalSerializer };
